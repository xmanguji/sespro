  ...
  <div class="h-full flex flex-row bg-sonicSilver">
    <div class="items-start flex gap-y-10 flex-col bg-indigo w-72">
      ...
      <div class="flex flex-col w-full">
        ...
        <div v-if="userRole == 'ROLE_ROOT' || userRole == 'ROLE_OWNER'"
          class="pl-8 flex w-full pb-1 pt-2 hover:bg-indigoLight border-b border-gray-700 cursor-pointer"
          @click="toPremises">
          <div class="flex items-end justify-center h-12">
        ...
        <div class="pl-8 flex w-full py-1 hover:bg-indigoLight border-b border-gray-700 cursor-pointer">
          <div class="flex items-end justify-center h-12">
    ...
    <div class="flex flex-col w-full h-full bg-white">
      <div class="flex w-full h-16 min-h-16 bg-indigoLight ">
        <div class="ml-6 font-normal text-2xl text-white flex flex-row my-3 items-center justify-start w-full">
          <ArrowLeft v-if="back" class="mr-2" @click="goBack" />
          <div class="flex flex-1">
            <h4>{{ headerTitle }}</h4>
          </div>
          <button v-if="isButtonAllowed" @click="onSave" class="mr-5 bg-blue-400 px-2 rounded">Save and Update</button>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    onSave() {
      this.$emit('click-event', 'Button clicked!'); // Emit the custom event 'click-event' and pass data to the parent
    }
  },
};
</script>
