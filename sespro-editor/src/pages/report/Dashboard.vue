      ...
      <div>
        <div class="p-3 flex flex-row w-full justify-between items-center mt-2 card">
          ...
          <div class="flex flex-row items-center">
            ...
            <div class="dx-field-value">
              <DxButton class="h-full" :width="90" text="Run" type="default" styling-mode="contained" :disabled="!(selectedTemplate && selectedLocationUuids.length > 0)"
          ...
          <div class="flex flex-row items-center">
            ...
            <div class="flex flex-row items-center">
              ...
              <div class="dx-field-value">
                ...
                :disabled="!(selectedLocationUuids && selectedGroup)"
          ...
          <div class="col-span-3 card">
            ...
            <div class="px-4 mt-4" v-for="(location, index) in complianceByLocationDataBest.slice(0, 10)" :key="index">
              <div class="justify-between w-full flex-row flex my-2"> 
                ...
                <div style="font-size: 13px;">{{ location.percentage }}</div>
              </div>
        ...
        <div class="flex mx-3 my-3 items-center justify-between w-full">
          <div class="text-xl font-semibold">
            ...
            <div class="flex flex-row items-center">
              ...
              <div class="dx-field-value">
                <DxButton class="h-full" :width="90" text="Run" type="default" styling-mode="contained"
                :disabled="!(selectedTemplate && selectedLocationUuids.length > 0)"
        ...
        <div class="grid gap-3 grid-cols-2">
          <div class="pb-2 card">
            ...
            <div class="px-4 mt-4" v-for="(section, index) in complianceSectionPerformace.slice(0, 10)" :key="index">
              <div class="justify-between w-full flex-row flex my-2">
                <div style="font-size: 14px;">{{ section.section }} </div>
                <div style="font-size: 13px;"> {{parseInt(section.percentage_passed)}}%</div>
              </div>
          ...
          <div class="pb-2 card">
            ...
            <div class="px-4 mt-4" v-for="(question, index) in complianceByQuestionDataWorst.slice(0, 10)" :key="index">
              <div class="justify-between w-full flex-row flex my-2">
                <div style="font-size: 14px;" class="text-left">{{ question.question }} </div>
                <div style="font-size: 13px;">{{parseInt(question.percentage)}}%</div>
              </div>
              <div :class="`h-1 w-full -mt-1`" :style="{ background: getGraphColor(index)[1] }">
                <div :class="`h-1`"
                  :style="{ width: (question.percentage * 100) + '%', background: getGraphColor(index)[0] }" />
              </div>
              <div class="flex-row flex justify-around pt-2">
                <div class="text-xs pt-2">Total <br/>{{ question.total }}</div>
                <div class="text-xs pt-2">Pass <br/>{{ question.pass }}</div>
                <div class="text-xs pt-2">Fail <br/>{{question.fail}} </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    onGetComplianceBySection(dataSource) {
      this.complianceSectionPerformace = dataSource;
    ...
    onGetComplianceByQuestion(dataSource) {
      this.complianceByQuestionDataWorst = dataSource;
      // const filteringData = this.selectedTemplate
      //   ? dataSource.filter(x => x.template_name === this.selectedTemplate.name)
      //   : dataSource;
    ...
    onGetComplianceByLocation(dataSource) {
      this.complianceByLocationDataBest = dataSource.map(item => {
        return ({...item, percentage: (item.avg_score * 100).toFixed(2) + '%' })
      })
      // const filteringData = this.selectedTemplate
      //   ? dataSource.filter(x => x.template_name === this.selectedTemplate.name)
      //   : dataSource;
      ...
      //   .slice()
      //   .sort((a, b) => a.score / a.val.length - b.score / b.val.length)
      ...
      const performanceLocationData = groupByDateLocation.map(i => {
        ...
        groupByDateLocation.forEach(data => {
          i[data.location_uuid] = parseInt(data.val.reduce((p, n) => {
            return p + Number(n.avg_score * 100)
          }, 0))
        })

        return i
      })
      this.performanceLocationData = performanceLocationData.sort((a, b) => {
    ...
    onRunReportComplianceBySection() {
      ...
      const params = `fromDate=${fromDateString}&toDate=${toDateString}&locationUuids=${this.selectedLocationUuids}&templateUuid=${templateUuid}`

      Promise.all([
        API.fetch_dashboard_section_performance_aggregated(this.token, params),
    ...
    onRunReportComplianceByLocation() {
      ...
      const premisesGroupUuid = this.selectedGroup == null ? '' : this.selectedGroup.uuid
      const templateUuid = this.selectedTemplate == null ? '' : this.selectedTemplate.uuid
      const params = `fromDate=${fromDateString}&toDate=${toDateString}&premisesGroupUuid=${premisesGroupUuid}&templateUuid=${templateUuid}`

      Promise.all([
        API.fetch_dashboard_location_performance(this.token, params),
        API.fetch_dashboard_number_of_audits_submitted(this.token, params),
        API.fetch_dashboard_average_score(this.token, params)
      ])
        .then((responses) => {
          const [locationPerformance, numberOfAudits, averageScore] = responses
          this.totalNumberOfAuditsSubmitted = numberOfAudits.data
          this.averageScore = averageScore.data
          this.onGetComplianceByLocation(locationPerformance.data); // audit scores
          loadingComponent.close();
        }).catch(error => {
          console.log(error)
          loadingComponent.close();
    ...
    onRunReportComplianceByQuestion() {
      ...
      Promise.all([
        API.fetch_dashboard_question_performance_aggregated(this.token, params),
      ])
        .then((responses) => {
          const [genericData] = responses
    ...
    onRunReportLocationScores() {
      ...
      const fromDateString = `${fromDate.getFullYear()}-${fromDate.getMonth() + 1}-${fromDate.getDate()}`
      const toDateString = `${toDate.getFullYear()}-${toDate.getMonth() + 1}-${toDate.getDate()}`
      const locationUuid = this.selectedLoactions.map(x => x.uuid);
      const templateUuid = this.selectedTemplate == null ? '' : this.selectedTemplate.uuid

      const params = `fromDate=${fromDateString}&toDate=${toDateString}&locationUuids=${locationUuid}&templateUuid=${templateUuid}`

      Promise.all([
        API.fetch_dashboard_location_monthly_data(this.token, params),
      ])
        .then((responses) => {
          const [genericData] = responses
