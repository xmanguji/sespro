<template>
  <AppLayout :back="false" headerTitle="Failures Report">
    <!-- <template v-slot:first>
      <b-tooltip label="Done" position="is-bottom">
        <button
          @click="onFinish"
          class="
            text-white
    ...
    <template>
      <div v-if="isData()">
        <div class="p-12">
          <Donut type="pie" :data="failuresData" class="mt-4" :options="options"></Donut>
        </div>

        <div class="section-lists">
          <div v-for="(label, index) in failuresData.labels" :key="index">
            <div v-if="getFailurePercent(failuresData.datasets[0].data[index]) !== 0
              " class="section-list-view border-b">
              <div class="list-first-column">
                <img class="logo" :src="getIconPath(failuresData.icons[index])" />
                <label>{{ label }}</label>
              </div>
      ...
      <div class="flex justify-center mt-12">
        <button @click="onFinish" class="text-white rounded-md bg-blue-500 px-6 py-3">
          Done
        </button>
      </div>
...
export default {
  name: "AuditGraph",
  components: { Donut, AppLayout, Close },
  computed: {
