<template>
  <AppLayout :back="true" :imageLogo="defaultImage" @back="onBack" headerTitle="Respro" :loading="loading">
    <div class="" v-if="isChallenge">
      <p class="subtitle text-left mt-3 text-gray-400" v-if="pageQuestions.length < 5">
        Answer {{ 5 - pageQuestions.length }} questions to complete this section
      </p>
    </div>
    <div v-if="isChallenge">
      <div class="questions-div" v-if="pageQuestions && pageQuestions.length > 0 && isChallenge">
        <div v-for="question in pageQuestions" :class="[
          'question-container',
          question.pass != null && question.pass === true ? 'passed' : '',
          question.pass != null && question.pass === false ? 'failed' : '',
        ]" :key="question.resultUuid">
          <p class="question-text" @click="onTapQuestion(question.resultUuid)">
            <span v-if="!challenge()">{{ question.order + 1 }}.</span>
            <vue-simple-markdown :source="question.text"></vue-simple-markdown>
          </p>
          <div :class="[
            'useraction-div', 'useraction-div-challenge',
          ]">
            <div class="radio-btn-cnt">
              <b-radio type="is-primary" :name="question.resultUuid" class="radio-btn-custom" :value="getState(question)"
                @input="pass(question)">Yes</b-radio>
              <b-radio type="is-danger" :name="question.resultUuid" class="radio-btn-custom radio-btn-margin"
                :value="getState(question)" @input="fail(question)">No</b-radio>
              <b-radio native-value="skipped" type="is-black"
                class="radio-btn-custom radio-btn-margin" :value="getState(question)"
                @input="skip(question)">N/A</b-radio>
              <img class="info-icon" src="../../assets/info_icon.png" @click="onShowHint(question)" />
            </div>
            <div v-if="(
              !hasSkipped(question) &&
              ...
              challenge()
              ">
              <input @keyup.enter="onKeyEnter(question, $event)"
                ...
                @change="saveNote(question, $event)" :value="getNote(question)" :placeholder="challenge() ? 'Answer' : 'Comments'
                  " />
              <div v-if="!challenge()" class="input-field" @click="onCamera(question)">
                <div class="input-field-photo text-lg ">
                  <img class="camera-icon logo" src="../../assets/images/icons/icon_camera.png" />
                  <label>{{
                    hasPhoto(question) ? "Retake Photo" : "Take a Photo"
                  }}</label>
                  <input :ref="'camera' + question.order" @change="onCapture(question)" class="hidden-camera-input"
                    type="file" capture="environment" accept="image/jpeg;image/jpg" />
                </div>
              </div>
              <div v-if="!challenge() && question.photo_url" style="text-align: center">
    ...
    <div v-else>
      ...
      <div class="questions-div" v-if="pageQuestions && pageQuestions.length > 0">
        <div v-for="question in pageQuestions" :class="[
          'question-container',
          question.pass != null && question.pass === true ? 'passed' : '',
          question.pass != null && question.pass === false ? 'failed' : '',
        ]" :key="question.uuid" :data-id="question.order">
          <p class="question-text" @click="onTapQuestion(question.uuid)">
            <span v-if="!challenge()">{{ question.order + 1 }}.</span>
            <vue-simple-markdown :source="question.text"></vue-simple-markdown>
          </p>
          <div :class="[
            ...
            <div class="radio-btn-cnt">
              <b-radio native-value="info" type="is-success" class="radio-btn-custom" :name="question.uuid"
                :value="getState(question)" @input="pass(question)">Yes</b-radio>
              <b-radio native-value="danger" type="is-danger" :name="question.uuid"
                class="radio-btn-custom radio-btn-margin" :value="getState(question)" @input="fail(question)">No</b-radio>
              <b-radio native-value="skipped" type="is-black"
                class="radio-btn-custom radio-btn-margin" :value="getState(question)" @input="skip(question)">N/A</b-radio>
              <img class="info-icon" src="../../assets/info_icon.png" @click="onShowHint(question)" />
              
            </div>
            <div v-if="(!hasSkipped(question) &&
              hasAnswered(question)) ||
              challenge()
              ">
              <input @keyup.enter="onKeyEnter(question, $event)"
    ...
    <div class="button-pick-question" v-if="isChallenge">
      <b-button type="button" class="is-large is-info" @click="onPickQuestion" v-if="pageQuestions.length < 5">{{
        pageQuestions.length > 0 ? "Pick Next Question" : "Pick A question"
      }}</b-button>
      <p class="is-size-7" style="padding: 1.5rem 0" v-if="pageQuestions.length > 0">
...
export default {
  ...
  methods: {
    challengesAnswered: function () {
      return this.category.questions.filter((q) => "note" in q).length;
    },
    challenge: function () {
      return "challenge" in this.category;
    },
    setQuestion(currentAuditIndex, category, question, result) {
      this.$store.commit("audit/SET_QUESTION", { currentAuditIndex, category, question, result });
    },
    setNote(currentAuditIndex, category, qIndex, value) {
      this.$store.commit("audit/SET_NOTE", { currentAuditIndex, category, qIndex, value });
    },
    skipQuestion(currentAuditIndex, category, question) {
      this.$store.commit("audit/SKIP_QUESTION", { currentAuditIndex, category, question });
    },
    setPicked(currentAuditIndex, category, qIndex, value) {
      this.$store.commit("audit/SET_PICKED", { currentAuditIndex, category, qIndex, value });
    ...
    pass(question) {
      ...
      try {
        this.setQuestion(this.currentAuditIndex, this.category.order, qindex, true);
        this.$forceUpdate();
      } catch (error) {

        if (error.name == 'QuotaExceededError') {
          this.$buefy.dialog.confirm({
            message: "Error: you exceeded storage limit please clear cache and try again",
    ...
    fail(question) {
      ...
      console.log('ddddddddddddd +==============> ', qindex)
      ...
      try {
        this.setQuestion(this.currentAuditIndex, this.category.order, qindex, false);
        this.$forceUpdate();
      } catch (error) {
    ...
    getState(question) {
      if (!this.hasAnswered(question)) {
        // console.log('here?nothing', question.pass, '====', question.text)
        return "nothing";
      } else if (this.hasPassed(question)) {
        // console.log('here?info', question.pass, '====', question.text)
        return "info";
      } else if (this.hasSkipped(question)) {
        // console.log('here?skipped', question.pass, '====', question.text)
        return "skipped";
      } else {
        // console.log('here?danger', question.pass, '====', question.text)
        return "danger";
      }
    },
    ...
    saveNote(question, event) {
      ...
      if (note !== "") {
        ...
        this.category.questions[qindex]["note"] = note;

        try {
          this.setNote(this.currentAuditIndex, this.category.order, qindex, note);
          this.$forceUpdate();
        } catch (error) {
    ...
    getNote(question) {
      ...
      if (Object.prototype.hasOwnProperty.call(q, "note")) {
        return q.note;
      } else {
        return null;
      }
    ...
    onPickQuestion() {
      const seletableQuestions = this.category.questions.filter(item => {
        return item && !Object.prototype.hasOwnProperty.call(item, "resultUuid")
      });
        ...
        events: {
          onSelectChallengeQuestion: (event, question) => {
            ...
            try {
              ...
              this.setPicked(this.currentAuditIndex, this.category.order, qIndex, true);
            } catch (error) {
    ...
    onTapQuestion(questionUuid) {
      if (this.tapHistory.uuid == questionUuid) {
        ...
        if (this.tapHistory.count == 3) {
          ...
          try {
            this.setPicked(this.currentAuditIndex, this.category.order, qIndex, false);
          } catch (error) {
  ...
  mounted() {
    ...
    this.updatePageQuestions();
    console.log('pass.category.order ===> ', this.category)
  },
};
</script>
...
.main-container {
  ...
  .questions-div {
    ...
    .question-container {
      ...
      &.passed {
        border-left: 0.4rem solid #59bc2d;
      }

      &.failed {
        border-left: 0.4rem solid #a1151a;
      }
